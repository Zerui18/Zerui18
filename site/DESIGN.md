...
- Links: GitHub `Zerui18` · LinkedIn `in/zerui-chen-241015204` · Google Scholar (`N3_D8QQAAAAJ`) · Résumé (`assets/resume.pdf`, the research variant) · `hi@zeruic.me`. The PDF is generated by `resume/build.sh` from `resume/content.yaml` + `resume.typ`, never hand-edited. The quant variant is built to `resume/out/` only and is not on the site.
